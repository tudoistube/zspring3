...
public class AuthInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, 
		...
		if(session.getAttribute("login") == null){
			...
			/*
			 * ...S.668p.자동로그인처리.
			 * ...현재 사용자가 HttpSession에 적당한 값이 없는 경우, loginCookie를 가지고 있는지 체크함.
			 * ...현재 사용자의 세션에 login이 존재하지 않지만, 쿠키중에 loginCookie가 존재할 경우,
			 * ...즉, 사용자가 이전에 로그인을 한 적이 있음.
			 */
			Cookie loginCookie = WebUtils.getCookie(request, "loginCookie");

			if(loginCookie != null) { 

				logger.info("z5.checkLoginBefore with session_key = " + loginCookie.getValue());
				MsgUserVO userVO = service.checkLoginBefore(loginCookie.getValue());

				if(userVO != null){
				  logger.info("z5.userVO = " + userVO.toString());					
