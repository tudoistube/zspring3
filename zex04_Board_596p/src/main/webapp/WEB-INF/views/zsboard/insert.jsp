...
<!-- ...202p. form::action 속성이 지정되지 않으면 현재 경로를 그대로 action의 대상 경로로 잡음.
     ...595p. form 태그의 id 에 'registerForm' 속성값을 설정함. 
-->
...
$("#registerForm").submit(function(event){
	...
	console.log("insert.jsp : registerForm submit : str = " + str);
	/*...595p.registerForm 내부의 맨 마지막에 아래와 같은 태그가 생성되어 서버쪽 컨트롤러에 전달함.
	     이때 각 파일은 'files[0]'과 같은 이름으로 추가되는데, 이 배열 표시를 이용해서 컨트롤러에서는 
	     BoardVO의 files파라미터를 수집하게 함.
	     <input type="hidden" name="files[0]" value="/2017/02/27/s_02bf25ac-6306-4157-bc73-0c1e2fd0b0cf_Tulips.jpg"> */
	that.append(str);

	/*...595p.that.get(0)의 get(0)은 순수한 DOM객체를 얻어내기 위해서 사용함. */
	that.get(0).submit();
});
</script>
