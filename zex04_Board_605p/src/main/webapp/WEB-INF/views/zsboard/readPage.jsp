...
<script>				
	$(document).ready(function(){
		...
		$("#modifyBtn").on("click", function(){
			...
			//...수정화면으로 이동(get).
			formObj.attr("action", "/z4b/zsboard/updatePage");
		...
		$("#deleteBtn").on("click", function(){
			...
			formObj.attr("action", "/z4b/zsboard/deletePage");
			formObj.submit();
		});
		...
		$("#listBtn").on("click", function(){
			...
			//...294p.내용조회후 다시 원래 목록 위치로 돌아가기 위한 정보추가.
			formObj.attr("method", "get");
			formObj.attr("action", "/z4b/zsboard/listPage");
			formObj.submit();		
		});
 ...
 * ...604p.첨부파일이 이미지인 경우, 원본파일의 경로를 특정한 div에 img 객체로
   ...
   ...★첨부파일 또는 첨부이미지가 있으면 mailbox-attachment-info 클래스의 div 갯수가
      3개 이상 (핸들바, 자바스크립트소스, 첨부파일갯수...) 생성됨.
	  uploadedList div 에 click 이벤트를 위임하여 첨부파일 또는 첨부 이미지가 있을때만
	  생성되는 미래에 생길 mailbox-attachment-info 의 click 이벤트 처리를 구현함.
 */
...
<script>
	...
	//...★ /z4b/replies/로 요청경로를 한 이유를 알아야 함.
	...
	$(".pagination").on("click", "li a", function(event){
		...
		getPage("/z4b/replies/"+bno+"/"+replyPage);
