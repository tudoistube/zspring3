...
public class SampleAdvice {
   ...
   * 
   * ...480p.특정 컨트롤러를 호출할때 전달되는 파라미터를 확인하기 위해
   *    JoinPoint 타입을 사용함.
   *    실행하면 콘솔에 아래와 같이 JoinPoint 를 통해 파라미터를 확인할 수 있음.
		INFO : org.joywins.aop.SampleAdvice - S.startLog.----------------------------
		INFO : org.joywins.aop.SampleAdvice - [MessageVO [mid=null, receiver_id=zuser01, sender_id=zuser02, message=You can do it!, open_date=null, send_date=null]]
		INFO : org.joywins.aop.SampleAdvice - E.startLog.----------------------------
   */
  @Before("execution(* org.joywins.service.MsgService*.*(..))")//...495p.주석.트랜잭션.
  //public void startLog() {//...commented since 480p.
  public void startLog(JoinPoint jp) {

    logger.info("----------------------------");
    logger.info("S.startLog.----------------------------");
    logger.info(Arrays.toString(jp.getArgs())); //...added since 480p.  
